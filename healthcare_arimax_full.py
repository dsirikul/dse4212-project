...
for ticker in tickers:
    ...
    # Drop any NaN values created during feature generation
    ...
    # Define endogenous (Close price) and exogenous (all engineered features + raw exogenous variables)
    endog_train = train_data['Close']
    exog_train = train_data[['Open', 'Low', 'High', '5_day_SMA', '20_day_SMA', '50_day_SMA', '14_day_RSI', 'Lagged_Close', 'ATR', 'Day_of_Week', 'Daily_Return', 'Volatility']]
    endog_test = test_data['Close']
    exog_test = test_data[['Open', 'Low', 'High', '5_day_SMA', '20_day_SMA', '50_day_SMA', '14_day_RSI', 'Lagged_Close', 'ATR', 'Day_of_Week', 'Daily_Return', 'Volatility']]

    # Fit ARIMAX model with exogenous variables
    print(f"Fitting ARIMAX model for {ticker} with exogenous variables...")
    model = ARIMA(endog_train, order=(1, 1, 1), exog=exog_train)
...
print("Data processing, ARIMAX modeling with feature engineering, residuals, and ACF plotting completed.")
